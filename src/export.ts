...
import { DesignObject } from './objects.ts';
import type { PathPoint, Line, TextObject, Frame, Path } from './objects.ts';
...
export class ExportManager {
    ...
    constructor(app: AppInterface) {
        ...
        this.scaleSelect = document.getElementById('export-scale') as HTMLSelectElement;
    ...
    async export(): Promise<void> {
        const format = (this.formatSelect?.value || 'png') as ExportFormat;
    ...
    async exportPNG(scale: number = 1): Promise<void> {
...
`;
    ...
    renderObjectToContext(ctx: CanvasRenderingContext2D, obj: DesignObject): void {
        ctx.save();
        ...
        switch (obj.type) {
            ...
            case 'ellipse':
                ...
                break;
            case 'line':
                this.renderLine(ctx, obj as unknown as Line);
                break;
            case 'text':
                this.renderText(ctx, obj as unknown as TextObject);
                break;
            case 'frame':
                this.renderFrame(ctx, obj as unknown as Frame);
                break;
            case 'path':
                this.renderPath(ctx, obj as unknown as Path);
                break;
            case 'group':
                const groupObj = obj as any;
                if (groupObj.children) {
                    groupObj.children.forEach((child: DesignObject) => this.renderObjectToContext(ctx, child));
                }
                break;
        }
    ...
    renderRectangle(ctx: CanvasRenderingContext2D, obj: DesignObject): void {
        const rectObj = obj as any;
        ctx.beginPath();
        if (rectObj.cornerRadius > 0) {
            const r = Math.min(rectObj.cornerRadius, obj.width / 2, obj.height / 2);
    ...
    renderEllipse(ctx: CanvasRenderingContext2D, obj: DesignObject): void {
        const cx = obj.x + obj.width / 2;
        const cy = obj.y + obj.height / 2;
    ...
    renderFrame(ctx: CanvasRenderingContext2D, obj: Frame): void {
        ...
        if (obj.fill) {
            ...
            if (frameObj.cornerRadius > 0) {
                ctx.roundRect(obj.x, obj.y, obj.width, obj.height, frameObj.cornerRadius);
            } else {
                ctx.rect(obj.x, obj.y, obj.width, obj.height);
            }
            ctx.fillStyle = obj.fill;
    ...
    objectToSVG(obj: DesignObject, indent: string = '  '): string {
        ...
        switch (obj.type) {
            ...
            case 'line':
                return this.lineToSVG(obj as unknown as Line, indent, transform, opacity);
            case 'text':
                return this.textToSVG(obj as unknown as TextObject, indent, transform, opacity);
            case 'frame':
                return this.frameToSVG(obj as unknown as Frame, indent, transform, opacity);
            case 'path':
                return this.pathToSVG(obj as unknown as Path, indent, transform, opacity);
            case 'group':
                const groupObj = obj as any;
                let svg = `${indent}<g${transform}${opacity}>\n`;
                if (groupObj.children) {
                    groupObj.children.forEach((child: DesignObject) => {
    ...
    rectangleToSVG(obj: DesignObject, indent: string, transform: string, opacity: string): string {
        const rectObj = obj as any;
        ...
        const rx = rectObj.cornerRadius > 0 ? ` rx="${rectObj.cornerRadius}"` : '';

        return `${indent}<rect x="${obj.x}" y="${obj.y}" width="${obj.width}" height="${obj.height}"${rx}${fill}${fillOpacity}${stroke}${transform}${opacity}/>\n`;
    }

    ellipseToSVG(obj: DesignObject, indent: string, transform: string, opacity: string): string {
        const cx = obj.x + obj.width / 2;
        const cy = obj.y + obj.height / 2;
    ...
    lineToSVG(obj: Line, indent: string, transform: string, opacity: string): string {
        const stroke = obj.stroke && obj.strokeWidth > 0 ?
    ...
    textToSVG(obj: TextObject, indent: string, transform: string, opacity: string): string {
    ...
    frameToSVG(obj: Frame, indent: string, transform: string, opacity: string): string {
        const frameObj = obj as any;
        let svg = `${indent}<g${transform}${opacity}>\n`;

        // Background
        if (obj.fill) {
            const rx = frameObj.cornerRadius > 0 ? ` rx="${frameObj.cornerRadius}"` : '';
    ...
    pathToSVG(obj: Path, indent: string, transform: string, opacity: string): string {
        if (obj.points.length < 2) return '';
