...
export class BooleanOperations {
     ...
     */
    static objectToPolygon(obj: DesignObject): Point[] {
        if (obj.type === 'rectangle') {
            const rectObj = obj as any;
            const { x, y, width, height, cornerRadius } = rectObj;
            ...
            return this.pathToPolygon(obj as any);
        }
        return [];
    }
     ...
     */
    static ellipseToPolygon(x: number, y: number, width: number, height: number, segments: number = 32): Point[] {
        const cx = x + width / 2;
        const cy = y + height / 2;
        const rx = width / 2;
        const ry = height / 2;
     ...
     */
    static roundedRectToPolygon(x: number, y: number, width: number, height: number, radius: number, segmentsPerCorner: number = 8): Point[] {
     ...
     */
    static pathToPolygon(path: { points: PathPoint[]; closed: boolean }, segmentsPerCurve: number = 8): Point[] {
        const points: Point[] = [];
     ...
     */
    static union(obj1: DesignObject, obj2: DesignObject): Path {
     ...
     */
    static subtract(obj1: DesignObject, obj2: DesignObject): Path {
        const poly1 = this.objectToPolygon(obj1);
        const poly2 = this.objectToPolygon(obj2);
     ...
     */
    static intersect(obj1: DesignObject, obj2: DesignObject): Path | null {
        const poly1 = this.objectToPolygon(obj1);
        const poly2 = this.objectToPolygon(obj2);
     ...
     */
    static exclude(obj1: DesignObject, obj2: DesignObject): Path {
     ...
     */
    static polygonToPath(polygon: Point[], fill: string | null | undefined, stroke: string | null | undefined): Path {
        const path = new Path({
            fill: fill,
            stroke: stroke,
     ...
     */
    static performOperation(app: AppInterface, operation: BooleanOperationType): Path | null {
